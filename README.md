...
Android Compose client for **ktor-bestgame 0.26.0**.
Server: branch `claude/tender-pasteur-a36kj2`, commit `5aa7605f6332b555b21bbe7a55bbd327c5ce69a7`.
...
- **An expedition (client 2.24.0, server 0.26.0).** A fifth tab, «Expedition», holds the campaign: chapter I is ten biome maps from level 1 to 20, each with two to four kinds of monster, opened one after another by reaching a map's exit. A map is carved anew from a seed every run and drawn in pseudo-isometry by libGDX from shapes alone; a thumb anywhere low on the screen is the stick, monsters wander and chase, and touching one opens the fight — automatic, each side striking at its own attack speed with damage types, criticals, armour, evasion, block, resistances, energy shield, leech, regeneration and stun. By the owner's decision the fight is the client's; what a kill brings is not: the server checks the map is open and the monster lives there, then rolls the experience, gold, orbs and equipment itself, more for a magic or rare monster. Life carries from fight to fight; a death ends the run and keeps the loot.
