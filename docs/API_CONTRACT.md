...
Создание персонажа — `POST /api/v1/character` с документом из трёх полей: `userId`, `name`, `classId`. Больше клиент не шлёт ничего: уровень, деньги, сумка, взятые узлы и все списки навыков — серверные, и он проставляет их сам. Шаблон формы редактора (`template(Catalog.CHARACTERS)`) рисует поля целиком и payload'ом не является; `GameApi.create` отклоняет лишнее до запроса и называет поля поимённо.
