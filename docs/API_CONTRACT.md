...
Сервер: ветка `claude/tender-pasteur-a36kj2`, коммит `f1dae3178b4b566297eab3022678f984b2bf772c` (ktor-bestgame 0.22.0).
...
Сервер реализует пятнадцать сфер: `ORB_OF_TRANSMUTATION, ORB_OF_AUGMENTATION, ORB_OF_ALTERATION, ORB_OF_ALCHEMY, REGAL_ORB, CHAOS_ORB, EXALTED_ORB, DIVINE_ORB, ORB_OF_ANNULMENT, ORB_OF_SCOURING, BLESSED_ORB, VAAL_ORB, ORB_OF_CHANCE, MIRROR_OF_KALANDRA, ORB_OF_REGRET`. Последняя добавлена в 0.16.0 и стоит особняком: на предмет её не применяют (`CR_009`), её тратит дерево навыков за возврат узла. Сфер POE, завязанных на сокеты, качество, карты и верстак, здесь нет.
