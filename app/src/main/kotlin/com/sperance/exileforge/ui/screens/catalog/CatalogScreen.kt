        ...
        item {
            ForgePanel {
                ...
                if(s.catalog == Catalog.EQUIPMENT)
                    OutlinedButton(enabled = !s.busy && !s.editorOpen, onClick = vm::randomItem, modifier = Modifier.fillMaxWidth()) {
                        Icon(ForgeGlyphs.Orb, null, modifier = Modifier.size(18.dp)); Spacer(Modifier.width(8.dp)); Text(tr("Получить случайный предмет", "Roll a random item"))
                    }
            }
        }
        val visible = s.items
