...
Русский, английский и китайский упрощённый. Каждая подпись, подсказка, сообщение об ошибке и проверка контракта существует на всех трёх: у клиента они лежат ключами в `core/src/main/resources/i18n/ui_{ru,en,zh}.json`, названия самих вещей приходят из словаря сервера. Список языков называет сервер — тот, что объявлен в его `locale/index.json`: язык, на котором сервер не умеет назвать ни одного предмета, не предлагают. Выбор живёт во вкладке «Аккаунт» и на экране входа рядом с адресом сервера, сохраняется в DataStore и применяется ко всем экранам. Первый запуск берёт язык устройства.

Russian, English and Simplified Chinese. Every label, hint, error and contract-validation message exists in all three: the client's own live as keys in `core/src/main/resources/i18n/ui_{ru,en,zh}.json`, and the names of things in the game come from the server's dictionary. The server decides which languages are offered — whatever its `locale/index.json` declares — because an interface in a language the server cannot name a single item in is half a translation. The choice sits on the Account tab and on the sign-in screen beside the server address, is stored in DataStore and applies to the whole interface. A first run follows the device's own language.
