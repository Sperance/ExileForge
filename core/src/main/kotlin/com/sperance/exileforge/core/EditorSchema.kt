...
fun schemaFields(schema: String, document: JsonObject = JsonObject(emptyMap())): List<FormField> = when(schema) {
    "reference" -> listOf(text("definitionId", "Модификатор"), num("revision", "Версия", 1, true, 1.0))
    "modifier" -> listOf(num("definitionRevision", "Версия определения", 1, true, 1.0), text("definitionId", "Модификатор", "life"), list("values", "Значения", InputSpec.Object("rolledValue")), num("tier", "Уровень (tier)", 1, true, 1.0), choice("source", "Источник", modifierSources, "PREFIX"), list("tags", "Теги", InputSpec.Text(tagSuggestions)))
