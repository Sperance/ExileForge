...
Бой идёт на изометрической карте: у каждой зоны своя, сгенерированная процедурно и всегда одинаковая — открытый берег, склеп из комнат и коридоров, цитадель с кольцевой стеной. По карте одновременно бродит пачка монстров зоны; герой ходит свайпом пальца, камера привязана к нему, а столкновение с монстром открывает бой. Один переключатель задаёт, сколько герой делает сам: «Вручную» — всё нажимает игрок, «Авто-удар» — герой сам бьёт того, до кого дотянулся, и сам пьёт флакон, «Авто-поход» — сам ищет врагов, убивает, подбирает добычу и вызывает босса, встав в круг призыва. Награда сервера падает на землю столбом света в цвет редкости, и её подбирают, просто пройдя по ней: по экипировке открывается карточка с модификаторами. Выбор зон, журнал и таблицы лута — в лагере. Считает по-прежнему только сервер: каждое число на карте — разница двух его снимков боя, а позиция решает лишь то, когда клиент отправит команду. Потерянный ответ восстанавливается повтором сохранённой команды, и карта не проигрывает его дважды. [Правила и управление](docs/COMBAT.md).
