...
Сервер: ветка `claude/tender-pasteur-a36kj2`, коммит `e9964a01445e76051a4ced23f8b5735a61e51f00` (ktor-bestgame 0.20.0).
...
| **Вход по устройству** | `GET /api/v1/user/login/byDeviceId?deviceId=` → `UserResponse`, `US_015` если такого устройства нет |
| **Регистрация по устройству** | `POST /api/v1/user/byDeviceId?deviceId=` → `UserResponse`, `US_013` если устройство уже заведено |
...
Вход по устройству — это и есть регистрация: сервер держит одну учётную запись на `device_id`, и отказ `US_015` означает «такого устройства ещё не было», а не ошибку. Клиент превращает его в `POST`, который заводит запись и отвечает ею же. Пароля в этом пути нет вовсе, поэтому и скрывать в журнале нечего: идентификатор устройства — не секрет, а имя.
