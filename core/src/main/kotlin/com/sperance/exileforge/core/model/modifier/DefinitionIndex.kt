...
/**
 * The definition by id, through an index built once per list (2.56.0).
 *
 ...
 */
fun List<ModifierDefinition>.definition(id: String): ModifierDefinition? {
    val held = index
    val ready = if (held != null && held.first === this) held.second else associateBy { it.id }.also { index = this to it }
    return ready[id]
}
