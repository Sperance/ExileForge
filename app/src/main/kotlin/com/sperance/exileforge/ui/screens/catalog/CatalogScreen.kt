    ...
    LazyColumn(Modifier.fillMaxSize(), contentPadding = PaddingValues(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        item {
            ScreenHeader(if (s.catalog == Catalog.CHARACTERS) ui("catalog.characters") else ui("catalog.stash"),
                ui("catalog.page_info", s.total, s.page + 1, maxOf(1, s.totalPages)),
                if (s.catalog == Catalog.CHARACTERS) ForgeGlyphs.Exile else ForgeGlyphs.Stash)
            if (s.adminTools) CatalogSwitch(s, vm)
            if (s.editorOpen) Text(ui("catalog.editor_busy"), color = Muted, style = MaterialTheme.typography.bodySmall)
        }
        item {
            ForgePanel {
                Engraved(ui("common.search"))
                OutlinedTextField(s.query, vm::query, label = { Text(ui("common.search_catalog")) },
                ...
                Button(enabled = !s.busy && s.signedIn, onClick = vm::applyFilters, modifier = Modifier.fillMaxWidth()) { Text(ui("common.find")) }
                EntitySpinner(ui("catalog.open_record"), "", when (s.catalog) {
        ...
        item {
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                OutlinedButton(enabled = !s.busy && s.page > 0, onClick = { vm.refresh(s.page - 1) }) { Text(ui("common.back")) }
