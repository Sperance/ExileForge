...
Бой идёт на изометрической площадке: герой ходит свайпом пальца, камера привязана к нему, а удары герой наносит сам — бьёт того, до кого дотянулся, пьёт флакон при низком здоровье и встаёт в защиту, когда флаконов нет. Добыча сервера падает на землю столбом света в цвет редкости, и её подбирают, просто пройдя по ней: по экипировке открывается карточка с модификаторами. Выбор зон, обычные мобы и боссы, журнал и таблицы лута — в лагере. Победа автоматически выдаёт опыт, золото, предметы и сферы. Считает по-прежнему только сервер: каждое число на поле — разница двух его снимков боя, а позиция решает лишь то, когда клиент отправит команду. Потерянный ответ восстанавливается повтором сохранённой команды, и поле не проигрывает его дважды. [Правила и управление](docs/COMBAT.md).
