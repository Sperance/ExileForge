...
- Локальная сборка пока не подтверждена: окружение потребовало загрузки Gradle и JDK; разрешение зависимостей проверяется. Актуальный результат сборки следует смотреть в Actions для ветки исправления.
