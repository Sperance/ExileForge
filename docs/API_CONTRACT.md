...
| Операция | Запрос |
|---|---|
| Страница | `GET /api/v1/{collection}/paged?page=0&size=20` → `{items,page,pageSize,totalItems,totalPages}` |
...
Фильтров у `/paged` нет. Поэтому фильтрованный поиск читает коллекцию целиком и сужает её на клиенте по уже записанным сервером полям; без фильтров используется серверная страница.
