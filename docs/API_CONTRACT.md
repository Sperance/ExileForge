...
Сервер: ветка `claude/tender-pasteur-a36kj2`, коммит `e1ff56c5bd6cfec6f703190bfb7ccb4f976d1a4c` (ktor-bestgame 0.48.0).
...
## Меньше запросов (с 0.48.0, ревизия API 5)

- `GET /static/index.json` — публичный манифест старта: `version`, `revision`, `routes` (по ним
  клиент решает, что умеет сервер), `locale`, `icons`, `portraits`, `world` (`hash`, `file`).
  Клиент читает его один раз на сервер и требует `revision >= 5`.
- `GET /world/world.json` — для вошедшего: `modifiers`, `classes`, `levels`, `tree`, `equipment`,
  `items` (сферы и материалы — по категории), `stats`. Хранится на устройстве по отпечатку из
  манифеста; `ETag` совпадает с ним.
- `GET /api/v1/character/view?characterId` — снимок героя `{version, parts}`, части `character`,
  `inventory`, `tree`, `bag`, `bench`, у каждой `{version, data}`. Заголовок `X-Hero-Parts:
  часть=отпечаток,...` (или `none`) — что у клиента уже есть: такие части не присылаются.
  `If-None-Match: "<version>"` → 304.
- Команды героя, кузницы, дерева, кампании, ремёсел, рецептов и промокодов, получившие
  `X-Hero-Parts`, отвечают `{success, data, hero}`; `hero` — тот же снимок. Нет `hero` —
  клиент перечитывает героя через `/view`.
...
## Портреты: SVG по отпечатку

С 0.29.0 у каждого класса и каждой формы монстра есть портрет — цветной SVG в поле 300×400 (три на четыре), лицо в круге (150, 165) радиусом 120. Публично, без конверта.

| Операция | Запрос |
|---|---|
| Манифест | `GET /portraits/index.json` → `{hash, width, height, portraits:{"class.WITCH":"<отпечаток>", "form.BAT":"…"}}` |
| Портрет | `GET /portraits/<раздел>/<код>.svg` — `class/WITCH.svg`, `form/BAT.svg`, `monster/<CODE>.svg` |

Клиент хранит каждый файл рядом с его отпечатком и качает заново только тот, чей отпечаток изменился. Монстр рисуется своим портретом, если он есть, иначе портретом формы, иначе рисунком клиента. SVG разбирает сам клиент (`PortraitSvg`), поэтому допустимы только `path`, `circle`, `ellipse`, `rect`, `g` и градиенты в `userSpaceOnUse` — остальное сервер не пропустит (`PortraitTest`).
...
{"type":"features.data.equipment.equipment_data.Weapon","code":"IRON_SKULLCAP","slot":"WEAPON_1H","rarity":"RARE",
 ...
 "durability":100,"fixedModifierIds":["<ModifierDefinition._id>"],"modifierPools":["weapon","local:attack_physical"],
 "pools":{"drop":100,"smith":100,"merchant":100}}
```
...
С 0.39.0 **пул — это тег**, а не список ссылок. `modifierPools` — теги, из которых шаблон роллит префиксы и суффиксы (пул слота и локальные пулы его базы: `helmet`, `local:armor`); кто в них состоит и с каким весом, говорит сам модификатор (`ModifierDefinition.pools`), и вес берётся из первого тега шаблона, в котором модификатор состоит, а вес 0 его исключает. `fixedModifierIds` — закреплённые модификаторы (implicit и строки уникалки), они попадают на каждую копию. `pools` — в каких пулах экипировки состоит сам шаблон: `drop`, `smith`, `merchant`, а уникалка — `unique:world`, `unique:chance`, `unique:smith` или `boss:<код босса>`. Что попадёт на экземпляр, решает сервер; клиент теги только показывает, фильтрует по ним каталог и правит в редакторе.
...
Продажа торговцу (`characterequipment/sell`, 0.17.0) уничтожает экземпляр и платит золотом. Цену назначает сервер: базовая цена шаблона × множитель редкости копии (с 0.41.0; раньше бралась редкость шаблона) × (1 + 0,15 за каждый выпавший аффикс) × `STOCK_GOLD` персонажа; клиент с 2.46.0 считает ту же сумму по правилу из `/system/stats` и показывает её на строке и карточке заранее. Последний множитель — задел: характеристика в перечислении есть, её пока никто не выдаёт, и пока она равна нулю цена от неё не меняется; появится модификатор «увеличение стоимости продажи» — заработает сам. Надетое (`CH_014`) и вставленное в гнездо (`CH_015`) не продаётся, как и не выставляется на аукцион. Ответ говорит, сколько дали и сколько золота стало.
...
Считается в два прохода: сначала база класса на уровне персонажа плюс дерево навыков, затем экипировка в порядке слотов — каждый следующий предмет проверяется по характеристикам, которые уже дали база, дерево и признанные рабочими предметы. Предмет, чьи требования не выполнены, остаётся в слоте, но не работает и попадает в `inactive` со своей причиной. **С клиента 2.46.0 (сервер 0.41.0) этот расчёт делает сам клиент** (`core/character/Sheet.kt`) — строка в строку по `CharacterStatsCalculator`, с теми же строками причин, — а `/inventory/stats` он больше не спрашивает (маршрут остаётся). Порядок подсчёта статов, порядок слотов и правило цены торговца отдаёт публичный `GET /system/stats`:

```json
{"stats":[{"stat":"STOCK_STRENGTH","order":10},…],"slots":["HELMET","BODY",…,"RING_2","JEWEL",…],
 "sell":{"affixShare":0.15,"rarity":{"COMMON":1.0,"UNCOMMON":1.5,"RARE":2.5,…}}}
```

По листу клиент заранее показывает, что изменит вещь, если её надеть, выключает «Надеть» для вещи, чьи требования не выполнены, и не отправляет покупку или трату, на которую не хватает сфер, золота, очков или материалов. Сервер по-прежнему проверяет каждую команду.

`unwearable` добавлено в 0.17.0: это вердикт по **всему справочнику экипировки** — какие шаблоны персонаж сейчас надеть не может и почему. Требование лежит на шаблоне, а не на экземпляре, поэтому один ответ помечает и арсенал, и чужой лот на витрине; с 2.46.0 клиент собирает этот список сам.
...
`GET /api/v1/characterequipment/bench?characterId=` — строки верстака, но только те, что герой уже
...
`POST /api/v1/characterequipment/craft?characterId=&inventoryId=&recipe=<code>` ставит ремесленный модификатор выбранного тира (значение внутри тира роллит сервер), `POST /api/v1/characterequipment/uncraft?characterId=&inventoryId=` снимает его за Orb of Scouring. Оба отвечают как `applyOrb`: `{messageKey, messageArgs, item}`. Правила сервера: предмет магический или выше, рецепт уже найден (`CR_024`), ремесленный модификатор на предмете один (`CR_015`), встаёт на свободное место своего вида (`CR_007`) и не рядом с модификатором своей группы (`CR_016`), строка подходит слоту (`CR_019`); снимать нечего — `CR_017`, строки нет — `CR_018`. Сферы списываются в той же транзакции, отказ ничего не стоит.

Рецепт находят на карте: убийство редкого монстра или босса (`kill`/`slayBoss`) с шансом 10% открывает
один незнакомый рецепт, не больше одного за заход карты (`start` сбрасывает счётчик). Тир решает
уровень локации — чем он ниже, тем выше тир. Найденный рецепт приходит в ответе как
`CampaignReward.recipeFound` (тот же `BenchRecipe`, или `null` почти всегда).
...
отдаёт поле мира и регионы с зонами-жетонами. У зоны — место `x`/`y` (ось `y` растёт вверх от старта), связи `from` (откуда в неё
ведут) и `to` (куда ведут из неё), `finale` у финала региона; зона открыта, когда убит босс хотя бы одной зоны из `from`:

```json
{"world":{"width":1100,"height":2000},
 "regions":[{"code":"REGION_1","label":{"x":850,"y":55},"zones":[{"code":"C1_TIDAL_SHORE","region":"REGION_1","x":470,"y":100,
  "from":[],"to":["C1_SHIP_GRAVEYARD","C1_BRINE_CAVES","C1_GULL_CLIFFS"],"finale":false,"biome":"SHORE","level":1,
...
| Босс убит | `POST /api/v1/character/campaign/boss?characterId=&mapCode=` → `CampaignReward` с `progress`: зона пройдена (0.67.0) и открывает соседние, выход открыт на час, повтор — `CP_008`; пока босс жив, `leave` — `CP_007` |
